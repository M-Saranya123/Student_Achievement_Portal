...
private JTextField eventNameField, eventDateField, statusField, certificateLinkField, cashPrizeField;
private JTextArea achievementArea; 
private String studentUsername;

public StudentPage(String studentUsername) { 
this.studentUsername = studentUsername; setTitle("Student Achievements");
...
// Text area to display achievements achievementArea = new JTextArea(); 
achievementArea.setEditable(false);
JScrollPane scrollPane = new JScrollPane(achievementArea);


// Add button actions
addAchievementButton.addActionListener(e -> addAchievement()); deleteAchievementButton.addActionListener(e -> deleteAchievement());

// Panel for input fields
JPanel topPanel = new JPanel();
topPanel.setLayout(new GridLayout(6, 2));
// Organize input fields topPanel.add(new JLabel("Event Name:")); 
...
// Method to add an achievement private void addAchievement() {
...
if (eventName.isEmpty() || eventDate.isEmpty() || status.isEmpty() || certificateLink.isEmpty() || cashPrize.isEmpty()) {
JOptionPane.showMessageDialog(null, "All fields must be filled out!"); return;
}


try (Connection conn = DBConnection.getConnection()) {
 
String sql = "INSERT INTO ACHIEVEMENTS (USERNAME, EVENT_NAME, EVENT_DATE, STATUS, CERTIFICATE_LINK, CASH_PRIZE) VALUES (?, ?, ?, ?, ?,
?)";
...
ps.executeUpdate(); 
loadAchievements();
// Clear input fields after insertion eventNameField.setText(""); 
...
// Method to delete an achievement private void deleteAchievement() {
String eventName = eventNameField.getText(); 
...
String sql = "DELETE FROM ACHIEVEMENTS WHERE USERNAME=? AND EVENT_NAME=?";
PreparedStatement ps = conn.prepareStatement(sql); ps.setString(1, studentUsername);
ps.setString(2, eventName); ps.executeUpdate(); loadAchievements();
} catch (Exception ex) { ex.printStackTrace();
}
}


// Method to load and display achievements private void loadAchievements() {
achievementArea.setText("");
try (Connection conn = DBConnection.getConnection()) {
String sql = "SELECT EVENT_NAME, EVENT_DATE, STATUS, CERTIFICATE_LINK, CASH_PRIZE FROM ACHIEVEMENTS WHERE USERNAME=?";
PreparedStatement ps = conn.prepareStatement(sql); 
ps.setString(1, studentUsername);
